...
export class TeamPicker {
 ...
 private draw(){
  ...
  if(this.lineup.selected.length<2){
   const empty=document.createElement('p');empty.className='remote-team-empty';empty.setAttribute('role','status');empty.textContent='Add a player below to complete your team. You can use the same character in both slots.';this.host.append(empty,this.community.element);
  }
