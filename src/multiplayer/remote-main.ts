...
function leaveCourt(){endReplay();pointPauseRemaining=0;gameEndReadyAt=null;document.body.classList.remove('remote-playing');settings.close();gameEnd.close();scene?.setRetainedTrajectory(null);}
...
el('remote-open-settings').onclick=()=>{clearTarget();el('remote-archive-game').textContent=session?.state?.archived?'Restore game':'Archive game';const rosterUrl=new URL('/?roster=1&from=multiplayer',location.origin);const matchId=new URLSearchParams(location.search).get('match');if(matchId)rosterUrl.searchParams.set('match',matchId);(el('remote-roster') as HTMLAnchorElement).href=rosterUrl.href;settings.showModal()};
...
function render(){
 ...
 if(!s){display=null;animation=[];shot=null;clearTarget();el('remote-choices').replaceChildren();el('remote-shot-options').hidden=true;return;}
...
el('remote-accept').onclick=()=>{if(!selectedInvite||!acceptTeam||accepting)return;accepting=true;const id=selectedInvite.id,button=el('remote-accept') as HTMLButtonElement;button.disabled=true;const key=`pickle-remote:${account}:${id}:accept`;void(async()=>{const c=await matchCredentials();const saved=browserStorage.getItem(key),request=saved?JSON.parse(saved):{team:await acceptTeam!.freshTeam()};browserStorage.setItem(key,JSON.stringify(request));const game=await remoteRequest<PublicMatch>(c.token,`/api/invitations/${id}/accept`,request);browserStorage.removeItem(key);await open(game.id)})().catch(e=>status(e.message)).finally(()=>{accepting=false;button.disabled=false});};
...
el('remote-archive-game').onclick=()=>{const s=session?.state;if(!s)return;const button=el('remote-archive-game') as HTMLButtonElement;button.disabled=true;void archiveGame(s.id,!s.archived).catch(e=>status(e.message)).finally(()=>button.disabled=false);};
...
function frame(now:number){
 try{if(!renderFailed&&scene&&display&&shot&&!document.hidden){
  ...
  scene.render(display,now/1000,shot,null);targetPicker?.sync(!el('remote-game').hidden&&!settings.open&&!gameEnd.open&&!pointCelebrating()&&!replayActive());
 }
