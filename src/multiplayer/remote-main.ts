...
function status(message:string){inviteStatus.textContent=el('remote-invite').hidden?'':message;el('remote-status').textContent=message;el('remote-live-status').textContent=message;if(message)document.querySelector<HTMLElement>('.remote-move-banner')!.hidden=false;}
...
function renderInvitations(){const host=el('remote-invitations');host.replaceChildren();if(gameFilter==='completed'||gameFilter==='archived')return;for(const invite of invitations){const incoming=invite.recipientId===account;if(gameFilter==='turn'&&(!incoming||invite.status!=='pending'))continue;const card=document.createElement('button');card.className='remote-game-card';card.dataset.state=invite.status==='declined'?'declined':'invitation';const badge=document.createElement('span');badge.className='remote-badge ready';badge.textContent=invite.status==='declined'?'DECLINED':incoming?'NEW GAME · INVITATION':'INVITATION SENT';const title=document.createElement('strong');title.textContent=invite.status==='declined'?`${invite.recipientName} declined`:incoming?`${invite.creatorName} invited you`:`Waiting for ${invite.recipientName}`;const copy=document.createElement('span');copy.className='remote-card-opponent';copy.textContent=invite.status==='declined'?'This game will not start.':incoming?'Choose your team and accept. You serve first.':'The game starts when they select their team and accept.';const details=document.createElement('span');details.className='remote-card-created';details.textContent=`${invite.court==='arizona'?'Arizona Desert':invite.court==='venice'?'The Beach':'The Forest'} · ${new Date(invite.createdAt).toLocaleString()}`;const action=document.createElement('span');action.className='remote-card-action';action.textContent=invite.status==='declined'?'View invitation ↗':incoming?'Review invitation ↗':'View invitation ↗';card.append(badge,title,copy,details,action);card.onclick=()=>void showInvitation(invite.id).catch(e=>status(e.message));if(invite.status==='declined'&&!incoming){
...
function renderGames(){
 ...
 const active=games.filter(g=>!g.archived&&g.status==='active');el('remote-game-count').textContent=`${active.length} in play`;
 ...
 if(!visible.length){const empty=document.createElement('div');empty.className='remote-empty';const title=document.createElement('h3');title.textContent=gameFilter==='archived'?'No archived games.':gameFilter==='turn'?'You’re all caught up.':gameFilter==='completed'?'The first finish is ahead.':'A fresh court awaits.';const copy=document.createElement('p');copy.textContent=gameFilter==='archived'?'Games you archive will appear here. You can restore them anytime.':gameFilter==='turn'?'Your opponents are up. Check back for your next shot.':gameFilter==='completed'?'Completed games will be waiting here.':'Choose a player and start your first game.';empty.append(title,copy);container.append(empty);}
 for(const game of visible){
  ...
  card.append(badge,names,opponents,score);if(created.textContent)card.append(created);card.append(ref,action);card.onclick=()=>void open(game.id).catch(e=>status(e.message));const entry=document.createElement('div');entry.className='remote-game-entry';
...
async function create(){
 ...
 const key=`pickle-remote:${account}:invitation`;const saved=browserStorage.getItem(key);const request:InviteRequest=saved?JSON.parse(saved):{requestId:playerId(),opponentId:(el('remote-opponent') as HTMLSelectElement).value,team:await createTeam.freshTeam(),court:selectedCourt,scoring:(el('remote-scoring') as HTMLSelectElement).value as InviteRequest['scoring']};browserStorage.setItem(key,JSON.stringify(request));
 await remoteRequest<Invitation>(c.token,'/api/invitations',request);browserStorage.removeItem(key);await lobby();status('Invitation sent. Your opponent will choose their team and serve first.');
}
...
el('remote-sign-out').onclick=()=>void(async()=>{await disableDevicePush();const {error}=await authClient()!.auth.signOut({scope:'local'});if(error){status('Could not sign out. Try again.');return;}showLogin();signup=false;authMode();status('Signed out on this device.');})().catch(e=>status(e.message));
