...
function renderGames(){
 ...
 for(const game of visible){
  ...
  card.append(heading,lineup,score);if(created.textContent)card.append(created);const rivalry=rivalryData(game.rivalry);if(rivalry?.current){const line=document.createElement('span');line.className='remote-card-rivalry';line.textContent=seriesLine(rivalry.current)+` · ${rivalry.current.games} games together`;card.append(line);}card.append(ref,action);card.onclick=()=>void (game.friendState==='pending'?shareMatch(game.id):open(game.id)).catch(e=>status(e.message));const entry=document.createElement('div');entry.className='remote-game-entry';
