import {inviteFriend,shareMatch,createYourPlayer} from './friend-flow';
...
import {mountTurnPrompt,setTurnPromptEligible,disableDevicePush} from '../pwa';
...
root.innerHTML=`<header class="remote-nav"><a id="remote-solo" class="remote-home-link" href="/?home=1" aria-label="Home" title="Home"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m3 10 9-7 9 7M5 9v12h5v-7h4v7h5V9"/></svg></a><a id="remote-lobby-roster" href="/?roster=1&amp;from=multiplayer">Roster ↗</a></header><div class="remote-meta"><p id="remote-status" role="status" aria-live="polite"></p></div><section id="remote-login" class="remote-new-game" hidden><p class="remote-eyebrow">WELCOME TO THE COURT</p><h1>Create your account</h1><p id="remote-auth-copy">Use a different email on each device and choose a password of at least 6 characters. No confirmation email is needed for this playtest.</p><form id="remote-login-form"><label id="remote-name-label">Player name<input id="remote-player-name" autocomplete="nickname" maxlength="32" required></label><label>Email<input id="remote-email" type="email" autocomplete="username" required></label><label>Password<input id="remote-password" type="password" autocomplete="new-password" minlength="6" maxlength="128" required></label><button class="remote-primary" id="remote-sign-in">Create account ↗</button></form><button id="remote-auth-mode" class="remote-quiet">Already registered? Sign in</button></section><section id="remote-name-setup" class="remote-new-game" hidden><h1>What should we call you?</h1><p>Your player name appears in games and the opponent list.</p><form id="remote-name-form"><label>Player name<input id="remote-existing-name" autocomplete="nickname" maxlength="32" required></label><button class="remote-primary">Save player name ↗</button></form></section><section id="remote-password-reset" class="remote-new-game" hidden><h1>Choose a new password</h1><p id="remote-reset-account"></p><p>For this playtest, use 6–128 characters. No capitals, numbers, or symbols required.</p><form id="remote-reset-form"><label>New password<input id="remote-new-password" type="password" autocomplete="new-password" minlength="6" maxlength="128" required></label><button class="remote-primary">Save password and play ↗</button></form></section><section id="remote-lobby" hidden><div class="remote-lobby-layout"><section class="remote-match-list"><div class="remote-section-heading"><h2>Your games</h2></div><div class="remote-filters" role="group" aria-label="Filter games"><button data-filter="active" aria-pressed="true">In play</button><button data-filter="turn" aria-pressed="false">Your turn</button><button data-filter="completed" aria-pressed="false">Finished</button><button data-filter="archived" aria-pressed="false">Archived</button></div><div id="remote-invitations"></div><div id="remote-games"></div></section><section class="remote-new-game remote-start-card"><div class="remote-lobby-court" aria-hidden="true"><span></span></div><h2>Who’s up next?</h2><p>Pick your team, invite a friend, and get a game going.</p><button id="remote-start-setup" class="remote-primary">New game <span aria-hidden="true">＋</span></button></section></div></section><section id="remote-setup" class="remote-new-game" aria-label="Set up a game" hidden><button id="remote-cancel-setup" class="remote-quiet">← Your games</button><div class="remote-setup-heading"><p class="remote-eyebrow">MEET ON COURT</p><h1>Set up your game</h1><p>Good company. A great game. Make it yours.</p></div><div class="remote-setup-fields"><label>Invite a player<select id="remote-opponent"></select></label><label>Scoring<select id="remote-scoring"><option value="rally-doubles">Rally scoring</option><option value="side-out-doubles">Side-out scoring</option></select></label></div><section class="remote-setup-section"><h2><span>01</span> Choose your team</h2><p class="remote-section-copy">Pick your player and partner from your roster.</p><div id="remote-create-team"></div></section><section class="remote-setup-section"><h2><span>02</span> Choose your court</h2><div class="remote-courts"><button type="button" data-remote-court="forest" aria-pressed="true"><img src="/assets/picklebash-select/locations/forest.jpg" alt=""><strong>The Forest</strong></button><button type="button" data-remote-court="venice" aria-pressed="false"><img src="/images/venice-sunset.svg" alt=""><strong>The Beach</strong></button><button type="button" data-remote-court="arizona" aria-pressed="false"><img src="/images/arizona-desert.svg" alt=""><strong>Arizona Desert</strong></button></div></section><div class="remote-setup-footer"><div class="remote-setup-summary"><strong>READY TO RALLY</strong><p id="remote-setup-summary"></p><p>First to 3 · Doubles · Equal skills</p></div><button id="remote-create" class="remote-primary">Send invitation ↗</button></div></section><section id="remote-invite" class="remote-new-game" hidden><button id="remote-invite-back" class="remote-quiet">← Your games</button><div class="remote-invite-heading"><p class="remote-eyebrow">GAME INVITATION</p><h1 id="remote-invite-title"></h1><p id="remote-invite-copy"></p></div><div id="remote-invite-preview"></div><h2 id="remote-accept-team-title">Choose your team</h2><div id="remote-accept-team"></div><div class="remote-invite-actions"><button id="remote-accept" class="remote-primary">Accept & start game ↗</button><button id="remote-decline" class="remote-quiet" hidden>Decline game</button><button id="remote-cancel-invite" class="remote-quiet" hidden>Cancel invitation</button><button id="remote-delete-invite" class="remote-quiet" hidden>Delete invitation</button></div></section><section id="remote-game" hidden><div id="remote-court"></div><div class="court-top"><div class="score" aria-label="Match score"><div class="score-row"><span id="remote-home-names"></span><b id="remote-home-score">0</b></div><div class="score-row score-row-away"><span id="remote-away-names"></span><b id="remote-away-score">0</b></div></div></div><nav class="remote-court-actions" aria-label="Match controls"><button id="remote-open-settings" class="gameplay-settings" aria-label="Game settings"><img src="/assets/picklebash-gameplay-ui/hud/picklebash-settings-button.png" alt=""></button><button id="remote-back" aria-label="Your games" title="Your games"><img src="/assets/picklebash-gameplay-ui/hud/picklebash-games-button.svg" alt=""></button><button id="remote-replay" class="remote-replay" aria-label="Replay last move" title="Replay last move" disabled><img src="/assets/picklebash-gameplay-ui/hud/picklebash-replay-button.svg" alt=""></button></nav><div class="remote-move-banner" role="status" aria-live="polite"><p id="remote-move"></p><small id="remote-live-status"></small><button id="remote-retry" hidden>Retry saved turn</button></div><dialog id="game-settings" aria-labelledby="settings-title"><div class="settings-heading"><div><h2 id="settings-title">Game settings</h2><p id="remote-rules"></p></div><button id="close-settings" aria-label="Close settings">✕</button></div><nav class="remote-menu-links" aria-label="Game menu"><a id="remote-roster" href="/?roster=1&amp;from=multiplayer">Roster</a></nav><label class="settings-toggle"><span>Flight guide<small>Show the ball path.</small></span><input id="remote-guides" type="checkbox" role="switch"></label><label class="settings-toggle"><span>Player names<small>Show names above players.</small></span><input id="remote-names" type="checkbox" role="switch" checked></label><label class="settings-toggle"><span>Playback speed<small>Animation speed on this device.</small></span><select id="remote-speed"><option value="1">1× · Normal</option><option value="2">2×</option><option value="3">3×</option></select></label><p id="remote-settings-account"></p></dialog></section><footer class="remote-account-footer"><p id="remote-account"></p><a id="remote-sign-out" href="/?multiplayer=1" hidden>Sign out</a></footer>`;
...
const claimPlayer=document.createElement('button');claimPlayer.className='remote-quiet';claimPlayer.textContent='Create your player';claimPlayer.hidden=true;claimPlayer.onclick=()=>void createYourPlayer(async()=>{await enter();}).catch(e=>status(e.message));el('remote-settings-account').before(claimPlayer);
const reshare=document.createElement('button');reshare.className='remote-quiet';reshare.textContent='Send Challenge';reshare.hidden=true;reshare.onclick=()=>{if(session)void shareMatch(session.state!.id).catch(e=>status(e.message));};el('remote-retry').after(reshare);
const nudgeHost=document.createElement('div');el('remote-settings-account').before(nudgeHost);
...
const challengeNext=document.createElement('button');challengeNext.textContent='Challenge a Friend';challengeNext.onclick=()=>{gameEnd.close();inviteFriend(open);};el('remote-end-back').before(challengeNext);
...
function syncGameEnd(){const s=session?.state;if(gameEnd.open)return;
 ...
 if(replayActive()||!s||s.status!=='completed'||animation.length||el('remote-game').hidden||document.hidden||settings.open){gameEndReadyAt=null;return;}
...
const replayDuration=()=>replaySegments.reduce((sum,segment)=>sum+playbackDuration(segment),0);
function endReplay(){replayOverlay.hidden=true;replayPlaying=false;replaySegments=[];document.body.classList.remove('remote-replaying');}
function drawReplay(){
 if(!display||!replaySegments.length)return;
 let time=replayTime,segment=replaySegments[replaySegments.length-1];
 for(const candidate of replaySegments){segment=candidate;if(time<=playbackDuration(candidate))break;time-=playbackDuration(candidate);}
...
el('close-replay').onclick=()=>{endReplay();skip();render();el('remote-replay').focus();};
...
el('remote-replay').onclick=()=>{
 ...
 clearTarget();pointPauseRemaining=0;animation=[];replaySegments=structuredClone(s.animation);display=structuredClone(s.display);replayTime=0;replayPlaying=true;replayOverlay.hidden=false;document.body.classList.add('remote-replaying');lastFrame=performance.now();scene?.setRetainedTrajectory(null);drawReplay();el('replay-toggle').focus();
};
el('remote-open-settings').onclick=()=>{clearTarget();const rosterUrl=new URL('/?roster=1&from=multiplayer',location.origin);const matchId=new URLSearchParams(location.search).get('match');if(matchId)rosterUrl.searchParams.set('match',matchId);(el('remote-roster') as HTMLAnchorElement).href=rosterUrl.href;openSettings()};
...
function status(message:string){createStatus.textContent=el('remote-setup').hidden?'':message;inviteStatus.textContent=el('remote-invite').hidden?'':message;el('remote-status').textContent=message;el('remote-live-status').textContent=message;if(message)document.querySelector<HTMLElement>('.remote-move-banner')!.hidden=false;}
...
function render(){
 ...
 document.querySelector<HTMLElement>('.remote-move-banner')!.hidden=hideCommentary&&!el('remote-live-status').textContent&&el('remote-retry').hidden;el('remote-settings-account').textContent=`Playing as ${accountLabel()}`;reshare.hidden=session?.state?.friendState!=='pending';if(!reshare.hidden)el('remote-move').textContent=`Waiting for ${session?.state?.invitedName} to join…`;else if(s.friendState==='cancelled')el('remote-move').textContent='This challenge is no longer available.';
...
async function open(id:string){trashTalk.reset();nudgeControl.update(null);selectedInvite=null;el('remote-invite').hidden=true;
 ...
 if(!scene){await preloadAthletes();scene=new CourtScene(el('remote-court'),()=>{});scene.setGuides(flightGuides);scene.setPlayerNames(playerNames);targetPicker=new CourtTargetPicker(remoteTargeting(()=>session,skip,status,()=>!pointCelebrating()&&!replayActive()),scene);}
...
async function lobby(){nudgeControl.update(null);selectedInvite=null;el('remote-invite').hidden=true;leaveCourt();
 ...
 (el('remote-create') as HTMLButtonElement).disabled=!config.creationEnabled||!config.testers.length;(el('remote-start-setup') as HTMLButtonElement).disabled=!config.creationEnabled;
...
async function refreshLobbyCards(){const c=await matchCredentials();[games,invitations]=await Promise.all([remoteRequest<PublicMatch[]>(c.token,'/api/matches'),remoteRequest<Invitation[]>(c.token,'/api/invitations')]);renderGames();renderInvitations();setTurnPromptEligible(games.length>0);}
function renderInvitations(){const host=el('remote-invitations');host.replaceChildren();if(gameFilter==='completed'||gameFilter==='archived')return;for(const invite of invitations){const incoming=invite.recipientId===account;if(gameFilter==='turn'&&(!incoming||invite.status!=='pending'))continue;const card=invitationCard(invite,incoming);card.onclick=()=>void showInvitation(invite.id).catch(e=>status(e.message));if(invite.status==='declined'&&!incoming){
...
async function archiveGame(id:string,archived:boolean){const c=await matchCredentials();await remoteRequest(c.token,`/api/matches/${id}/archive`,{archived});if(settings.open)settings.close();await lobby();status(archived?'Game archived. Restore it from Archived. Your opponent’s game is unchanged.':'Game restored to your list.');}
function renderGames(){
 ...
 if(!visible.length&&gameFilter!=='active'&&!(gameFilter==='turn'&&invitations.some(i=>i.recipientId===account&&i.status==='pending'))){const empty=document.createElement('div');empty.className='remote-empty';const title=document.createElement('h3');title.textContent=gameFilter==='archived'?'No archived games.':gameFilter==='turn'?'You’re all caught up.':gameFilter==='completed'?'The first finish is ahead.':'No games here yet.';const copy=document.createElement('p');copy.textContent=gameFilter==='archived'?'Games you archive will appear here. You can restore them anytime.':gameFilter==='turn'?'Your opponents are up. Check back for your next shot.':gameFilter==='completed'?'Completed games will be waiting here.':'Choose a player and start your first game.';empty.append(title,copy);container.append(empty);}
 for(const game of visible){
  const card=document.createElement('button');card.className='remote-game-card';const yours=game.currentTeam===game.viewerTeam,done=game.status==='completed';card.dataset.state=done?'finished':yours?'ready':'waiting';
  const badge=document.createElement('span');badge.className='remote-badge '+(done?'finished':yours?'ready':'waiting');badge.textContent=game.friendState==='pending'?`Waiting for ${game.invitedName}`:game.friendState==='cancelled'?'Challenge cancelled':done?'Finished':yours?'Your turn':'Their turn';
  const names=document.createElement('strong');const own=game.viewerTeam==='home'?['you','partner'] as const:['opponent-left','opponent-right'] as const;names.textContent=game.friendState?`${config?.selfName??'You'} vs. ${game.friendState==='pending'?game.invitedName:opponentLabel(game)}`:`vs ${opponentLabel(game)}`;
...
async function create(){
 ...
 const key=`pickle-remote:${account}:invitation`;
 const freshRequest=async():Promise<InviteRequest>=>({requestId:playerId(),opponentId:(el('remote-opponent') as HTMLSelectElement).value,team:await createTeam!.freshTeam(),court:selectedCourt,scoring:(el('remote-scoring') as HTMLSelectElement).value as InviteRequest['scoring']});
 ...
 await lobby();status('Invitation sent. Your opponent will choose their team and serve first.');
}
let selectedCourt:'forest'|'venice'|'arizona'='forest';
for(const button of Array.from(document.querySelectorAll<HTMLButtonElement>('[data-remote-court]')))button.onclick=()=>{selectedCourt=button.dataset.remoteCourt as typeof selectedCourt;for(const other of Array.from(document.querySelectorAll('[data-remote-court]')))other.setAttribute('aria-pressed',String(other===button));setupSummary();};
function setupSummary(){const select=el('remote-opponent') as HTMLSelectElement;el('remote-setup-summary').textContent=`${config?.selfName??'You'} vs ${select.selectedOptions[0]?.textContent??'Choose a player'} · ${selectedCourt==='arizona'?'Arizona Desert':selectedCourt==='venice'?'The Beach':'The Forest'}`;}
const existingPlayer=document.createElement('button');existingPlayer.className='remote-quiet';existingPlayer.textContent='Challenge an existing PickleBash player';el('remote-start-setup').after(existingPlayer);
el('remote-start-setup').textContent='Invite a Friend';el('remote-start-setup').onclick=()=>inviteFriend(open);
existingPlayer.onclick=()=>{el('remote-lobby').hidden=true;el('remote-setup').hidden=false;createTeam=new TeamPicker(el('remote-create-team'));setupSummary();el('remote-opponent').focus();};
el('remote-opponent').onchange=setupSummary;
el('remote-cancel-setup').onclick=()=>void lobby().catch(e=>status(e.message));
el('remote-create').onclick=()=>{const button=el('remote-create') as HTMLButtonElement;if(button.disabled)return;button.disabled=true;button.textContent='Sending invitation…';button.setAttribute('aria-busy','true');status('Preparing your team and sending invitation…');void create().catch(e=>status(e.message)).finally(()=>{button.disabled=!config?.creationEnabled||!config.testers.length;button.textContent='Send invitation ↗';button.removeAttribute('aria-busy');});};
...
function frame(now:number){
 try{if(!renderFailed&&scene&&display&&shot&&!document.hidden){
  ...
  scene.render(display,now/1000,shot,null);trashTalk.frame(scene,display,replayActive()?replayTime:null,!el('remote-game').hidden&&!settings.open&&!gameEnd.open);targetPicker?.sync(!el('remote-game').hidden&&!settings.open&&!trashTalk.isOpen&&!gameEnd.open&&!pointCelebrating()&&!replayActive());
 }
...
authClient()?.auth.onAuthStateChange((_event,s)=>{claimPlayer.hidden=!s?.user.is_anonymous;if(account&&s?.user.id!==account){showLogin();status('Sign in to continue with this account.');}});
async function enter(){const client=authClient();if(!client){showLogin();status('Cloud accounts are not configured.');return;}
 ...
 const marker='pickle-email-accounts-v1';if(!browserStorage.getItem(marker)){await client.auth.signOut({scope:'local'});browserStorage.setItem(marker,'1');const clean=new URL(location.href);clean.hash='';history.replaceState(null,'',clean);showLogin();status('Sign in or create an account for this device.');return;}
 ...
 const {data:{session:auth}}=await client.auth.getSession();if(!auth){showLogin();status('');return;}claimPlayer.hidden=!auth.user.is_anonymous;accountEmail=auth.user.email??'';if(!auth.user.user_metadata?.player_name){showLogin();el('remote-login').hidden=true;el('remote-name-setup').hidden=false;status(`Signed in as ${accountEmail}`);return;}el('remote-name-setup').hidden=true;const inviteId=new URLSearchParams(location.search).get('invite');if(inviteId&&/^[a-f0-9-]{36}$/i.test(inviteId)){await lobby();await showInvitation(inviteId);return;}const matchId=new URLSearchParams(location.search).get('match');if(matchId&&/^[a-f0-9-]{36}$/i.test(matchId))await open(matchId);else await lobby();}
...
el('remote-sign-out').onclick=event=>{event.preventDefault();void(async()=>{await disableDevicePush();const {error}=await authClient()!.auth.signOut({scope:'local'});if(error){status('Could not sign out. Try again.');return;}showLogin();signup=false;authMode();status('Signed out on this device.');})().catch(e=>status(e.message));};
...
try{await enter();}catch(e){if(e instanceof RemoteError&&e.status===401)showLogin();status((e as Error).message);}
