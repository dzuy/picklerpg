...
el('remote-replay').onclick=()=>{
 ...
 clearTarget();animation=structuredClone(s.animation);display=structuredClone(s.display);shot=presentation(display);playbackElapsed=0;animationStart=performance.now();lastFrame=animationStart;
};
...
function render(){
 ...
 if(shownVersion!==s.version){
  ...
  animation=structuredClone(s.animation);animationStart=performance.now();playbackElapsed=0;lastFrame=animationStart;
 }
 if(!animation.length&&display)shot=presentation(display);
 ...
 targetPicker?.sync(!settings.open);
...
function skip(){animation=[];if(session?.state){display=structuredClone(session.state.display);shot=presentation(display);}syncGameEnd();}
async function open(id:string){selectedInvite=null;el('remote-invite').hidden=true;
 ...
 if(!scene){await preloadAthletes();scene=new CourtScene(el('remote-court'),()=>{});scene.setGuides(flightGuides);scene.setPlayerNames(playerNames);targetPicker=new CourtTargetPicker(remoteTargeting(()=>session,skip,status),scene);}
...
function renderInvitations(){const host=el('remote-invitations');host.replaceChildren();if(gameFilter==='completed')return;for(const invite of invitations){const incoming=invite.recipientId===account;if(gameFilter==='turn'&&!incoming)continue;const card=document.createElement('button');card.className='remote-game-card';const badge=document.createElement('span');badge.className='remote-badge ready';badge.textContent=incoming?'NEW GAME · INVITATION':'INVITATION SENT';const title=document.createElement('strong');title.textContent=incoming?`${invite.creatorName} invited you`:`Waiting for ${invite.recipientName}`;const copy=document.createElement('span');copy.className='remote-card-opponent';copy.textContent=incoming?'Choose your team and accept. You serve first.':'The game starts when they select their team and accept.';const details=document.createElement('span');details.className='remote-card-created';details.textContent=`The Forest · ${new Date(invite.createdAt).toLocaleString()}`;card.append(badge,title,copy,details);card.onclick=()=>void showInvitation(invite.id).catch(e=>status(e.message));host.append(card);}}
async function showInvitation(id:string){const c=await matchCredentials();const invite=await remoteRequest<Invitation>(c.token,`/api/invitations/${id}`);if(invite.status==='accepted'&&invite.matchId){await open(invite.matchId);return;}selectedInvite=invite;el('remote-lobby').hidden=true;el('remote-setup').hidden=true;el('remote-invite').hidden=false;const url=new URL(location.href);url.searchParams.set('invite',id);url.searchParams.delete('match');history.replaceState(null,'',url);const incoming=invite.recipientId===account;
 ...
 el('remote-accept-team').hidden=!incoming;el('remote-accept').hidden=!incoming;if(incoming)acceptTeam=new TeamPicker(el('remote-accept-team'));status('');}
