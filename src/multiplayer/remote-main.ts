...
el('remote-accept').onclick=()=>{if(!selectedInvite||!acceptTeam||accepting)return;accepting=true;const id=selectedInvite.id,button=el('remote-accept') as HTMLButtonElement;button.disabled=true;const key=`pickle-remote:${account}:${id}:accept`;void(async()=>{const c=await matchCredentials();const saved=localStorage.getItem(key),request=saved?JSON.parse(saved):{team:await acceptTeam!.freshTeam()};localStorage.setItem(key,JSON.stringify(request));const game=await remoteRequest<PublicMatch>(c.token,`/api/invitations/${id}/accept`,request);localStorage.removeItem(key);await open(game.id)})().catch(e=>status(e.message)).finally(()=>{accepting=false;button.disabled=false});};
