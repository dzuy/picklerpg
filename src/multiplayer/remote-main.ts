...
function challengeTeam(team?:LobbyTeam){
 ...
 const heading=el('remote-setup').querySelector('h1')!;heading.textContent='Play with a Friend';
 el('remote-opponent-field').hidden=true;
 el('remote-friend-name-field').hidden=false;
 (el('remote-create') as HTMLButtonElement).disabled=team?!config?.creationEnabled:false;
 createTeam=new TeamPicker(el('remote-create-team'),undefined,undefined,true);el(team?'remote-cancel-setup':'remote-friend-name').focus();
}
...
async function refreshLobbyCards(){if(accepting)return;renderGames();if(!account||!config)return;const c=await matchCredentials();[games,invitations]=await Promise.all([remoteRequest<PublicMatch[]>(c.token,'/api/matches'),remoteRequest<Invitation[]>(c.token,'/api/invitations')]);renderGames();renderInvitations();}
...
async function create(){
 ...
 if(inviteByLink){
  ...
  browserStorage.removeItem(key);status('');await lobby();teamLobby?.selectTab('games');shareChallenge(challenge);showTurnPromptAfterInvite();return;
 }
...
function frame(now:number){
 try{if(!renderFailed&&scene&&display&&shot&&!document.hidden&&!el('remote-game').hidden){
  ...
  scene.render(display,now/1000,shot,session?.state?.status==='active'&&!replayActive()&&!pointCelebrating()?session.state.serveCall:null);trashTalk.frame(scene,display,replayActive()?replayTime:null,!el('remote-game').hidden&&!settings.open&&!gameEnd.open);targetPicker?.sync(!el('remote-game').hidden&&!settings.open&&!trashTalk.isOpen&&!gameEnd.open&&!pointCelebrating()&&!replayActive());
 }
