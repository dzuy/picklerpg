...
el('remote-open-settings').onclick=()=>{clearTarget();const rosterUrl=new URL('/?roster=1&from=multiplayer',location.origin);const matchId=new URLSearchParams(location.search).get('match');if(matchId)rosterUrl.searchParams.set('match',matchId);(el('remote-roster') as HTMLAnchorElement).href=rosterUrl.href;settings.showModal()};
