...
import {CloudPlayerSync} from '../cloud-players';
...
const claimPlayer=document.createElement('button');claimPlayer.className='remote-quiet';claimPlayer.textContent='Create your player';claimPlayer.hidden=true;claimPlayer.onclick=()=>void createYourPlayer(async()=>{await enter();teamLobby?.selectTab('profile');}).catch(e=>status(e.message));el('remote-settings-account').before(claimPlayer);
...
const rivalryCompletion=document.createElement('section');rivalryCompletion.className='rivalry-completion';el('remote-end-rule').after(rivalryCompletion);
...
function renderCompletion(s:PublicMatch){
 ...
 gameEnd.querySelector('.game-end-subtitle')!.textContent=s.endedEarly?'A player left this game.':'Ready for the next one?';
...
function openRoster(){
 if(!roster){
  ...
  roster=new PlayerCreator(()=>openGameSurface(),()=>{},(library,change)=>sync.save(library,change));
...
function challengeTeam(team?:LobbyTeam){
 ...
 select.disabled=!!team;if(team){if(!Array.from(select.options).some(o=>o.value===team.id)){const option=new Option(team.name,team.id);select.add(option);}select.value=team.id;}
 el('remote-lobby').hidden=true;el('remote-setup').hidden=false;
 const heading=el('remote-setup').querySelector('h1')!;heading.textContent=team?`Play with ${team.manager}`:'Create a Game';
 el('remote-opponent-field').hidden=true;
 el('remote-friend-name-field').hidden=!!team;
 (el('remote-create') as HTMLButtonElement).disabled=team?!config?.creationEnabled:false;
 createTeam=new TeamPicker(el('remote-create-team'),undefined,teamDirectory?.self.players);el(team?'remote-cancel-setup':'remote-friend-name').focus();
}
async function lobby(){el('remote-start-setup').onclick=()=>challengeTeam();existingPlayer.onclick=()=>teamLobby?.selectTab('friends');nudgeControl.update(null);selectedInvite=null;el('remote-invite').hidden=true;leaveCourt();
...
window.addEventListener('game-surface-closed',()=>void refreshLobbyCards().catch(e=>status(e.message)));
function renderGames(){
 ...
 for(const game of visible){
  ...
  card.append(badge,lineup,score);if(created.textContent)card.append(created);const rivalry=rivalryData(game.rivalry);if(rivalry?.current){const line=document.createElement('span');line.className='remote-card-rivalry';line.textContent=seriesLine(rivalry.current)+` · ${rivalry.current.games} games together`;card.append(line);}card.append(ref,action);card.onclick=()=>void (game.friendState==='pending'?shareMatch(game.id):open(game.id)).catch(e=>status(e.message));const entry=document.createElement('div');entry.className='remote-game-entry';
  const archive=document.createElement('button');archive.className='remote-quiet remote-archive-action';archive.textContent=game.archived?'Restore game':'Archive game';archive.setAttribute('aria-label',`${game.archived?'Restore':'Archive'} game vs ${opponentLabel(game)}`);archive.title='Only changes your game list';archive.onclick=()=>{archive.disabled=true;void archiveGame(game.id,!game.archived).catch(e=>{status(e.message);archive.disabled=false})};entry.append(game.friendState==='pending'?pendingInviteCard(card,async()=>{const c=await matchCredentials();const challenge=await remoteRequest<{token:string}>(c.token,`/api/multiplayer/challenge-for-match/${game.id}`);return new URL(`/challenge/${challenge.token}`,location.origin).href;},game.invitedName??'your friend'):card,archive);(game.friendState==='pending'?waiting:container).append(entry);
 }
}
...
async function create(){
 ...
 if(inviteByLink){
  ...
  const selection={name,team:await createTeam.freshTeam(),court:selectedCourt,target:(el('remote-target') as HTMLInputElement).valueAsNumber,scoring:(el('remote-scoring') as HTMLSelectElement).value};
  ...
  browserStorage.removeItem(key);status('');await lobby();teamLobby?.selectTab('games');shareChallenge(challenge);return;
 }
 const key=`pickle-remote:${account}:invitation:${(el('remote-opponent') as HTMLSelectElement).value}`;
 const freshRequest=async():Promise<InviteRequest>=>({requestId:playerId(),opponentId:(el('remote-opponent') as HTMLSelectElement).value,team:await createTeam!.freshTeam(),court:selectedCourt,target:(el('remote-target') as HTMLInputElement).valueAsNumber,scoring:(el('remote-scoring') as HTMLSelectElement).value as InviteRequest['scoring']});
 ...
 status('');
 await lobby();teamLobby?.selectTab('games');
}
...
el('remote-create').onclick=()=>{const button=el('remote-create') as HTMLButtonElement;if(button.disabled||!(el('remote-target') as HTMLInputElement).reportValidity()||(inviteByLink&&!(el('remote-friend-name') as HTMLInputElement).reportValidity()))return;button.disabled=true;button.textContent='Starting game…';button.setAttribute('aria-busy','true');status('Preparing your team and sending invitation…');void create().catch(e=>status(e.message)).finally(()=>{button.disabled=inviteByLink?false:!config?.creationEnabled;button.textContent='Start Game';button.removeAttribute('aria-busy');});};
...
function showOpenLobby(owner=''){
 ...
 const privateSetup=()=>{teamLobby?.selectTab('profile');signInDialog(async()=>{await enter();teamLobby?.selectTab('profile');status('');});};
 el('remote-start-setup').onclick=privateSetup;existingPlayer.onclick=privateSetup;teamDirectory=null;renderTeamLobby();
}
async function enter(){const client=authClient();if(!client){showOpenLobby();return;}
...
el('remote-sign-out').onclick=event=>{event.preventDefault();void signOutAccount().catch(()=>status('Could not sign out. Try again.'));};
