...
export async function remoteRequest<T>(token:string,path:string,body?:unknown):Promise<T>{
 ...
 let value;try{value=await response.json()}catch{throw new RemoteError(503,'unavailable','Multiplayer is temporarily unavailable. Please reload the page and try again.');}
