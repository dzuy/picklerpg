...
export function pendingInviteCard(card:HTMLButtonElement,link:()=>Promise<string>,recipient:string,incoming=false){
 ...
 const nudge=document.createElement('button');nudge.type='button';nudge.textContent='Nudge';nudge.title=incoming?'This invitation is waiting for you to accept.':`Share a reminder with ${recipient}`;nudge.disabled=incoming;
