...
function showSignIn(copy='Sign in to return to this match.'){
 button.hidden=true;signIn.hidden=true;switchPlayer.hidden=true;identityCopy.hidden=true;form.hidden=false;message.textContent=copy;email.focus();
}
signIn.onclick=()=>showSignIn();
document.querySelector<HTMLButtonElement>('#challenge-login-back')!.onclick=()=>{form.hidden=true;button.hidden=false;signIn.hidden=!accepted||needsSignIn;message.textContent='';};
