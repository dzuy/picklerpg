...
export class FriendSearch {
 ...
 private inviteLink=document.createElement('button');
 ...
 constructor(private input:HTMLInputElement,private teams:()=>LobbyTeam[],private select:(team:LobbyTeam|null)=>void,private portrait:(team:LobbyTeam)=>string=()=>''){
  ...
  this.inviteLink.type='button';this.inviteLink.className='remote-quiet';this.inviteLink.textContent='Invite by link — no account needed';input.after(this.inviteLink);this.inviteLink.addEventListener('click',()=>{this.select(null);this.close();this.hint.textContent='Guest invitation: share the link. They can accept without registering.';});
 ...
 reset(team?:LobbyTeam){this.input.value=team?.manager??'';this.select(team??null);this.hint.textContent=team?`${team.manager} must sign in to this account to accept. Or choose Invite by link for a guest.`:'';this.close();}
 private search(){
  ...
  this.hint.textContent=this.input.value.trim()?(this.matches.length?'Select an account for a sign-in invitation, or invite by link without an account.':'Guest invitation: you’ll get a link they can accept without registering.'):'';
 }
 ...
 private choose(team:LobbyTeam){this.input.value=team.manager;this.select(team);this.close();this.hint.textContent=`${team.manager} must sign in to this account to accept. Or choose Invite by link for a guest.`;}
