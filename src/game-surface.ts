...
export function openGameSurface(path='/?newgame=1'){
 if(surface)return;
 ...
 const message=(event:MessageEvent)=>{if(event.origin===location.origin&&event.source===frame.contentWindow&&event.data===CLOSE)close();};
