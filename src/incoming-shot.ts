...
export function incomingShotLabel(intent:ShotIntent|undefined,serving=false,popUp=false):string|null{
 if(serving)return 'Your serve';
 if(!intent)return null;
 if(popUp)return 'High pop-up incoming';
 ...
 const names:Record<ShotIntent['type'],string>={serve:'serve',return:'return',drive:'drive',drop:'drop',dink:'dink',reset:'reset',volley:'volley',counter:'counter',block:'block',overhead:'overhead smash',lob:'lob',flick:'flick'};
