...
export function installAccountControls(cloud:CloudPlayerSync,roster:()=>{id:string;name:string}[]=()=>[]){
 ...
 type Result={id:string;home_names:string;away_names:string;home_score:number;away_score:number;ended_early?:boolean;participants?:MatchParticipant[];difficulty?:string;target?:number};
 ...
 return {async completed(score:object,result:Omit<Result,'id'> & {id?:string}){
  ...
  try{const results=pending();const id=result.id??crypto.randomUUID();if(!results.some(row=>row.id===id))results.push({...result,id});browserStorage.setItem(pendingKey(),JSON.stringify(results));if(message)message.textContent='Saving match…';await retry();window.dispatchEvent(new CustomEvent('account-xp-saved',{detail:{gameId:id}}));if(message)message.textContent='Match saved · View it in Settings → Match history';}
