...
export class PlayerCreator {
 ...
 private setupAppearancePages(){
  ...
  track.addEventListener('pointerdown',event=>{
   ...
   if(event.target instanceof Element&&event.target.closest('input,select,textarea,.creator-choices'))return;
 ...
 private updateSummary(){const {meters,estimatedDupr}=summarizeSkills(this.draft.skills);
  ...
  this.el('[data-budget-help]').textContent=left?'Assign points or move them between skills anytime.':'';
  this.el('[data-budget-help]').hidden=!left;
 ...
 private paginateRoster(){
  ...
  const available=cards.filter(card=>card.closest('.community-section')&&!card.closest('.community-category-row'));
