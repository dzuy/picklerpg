...
export const SHOT_INTENT_SCHEMA={type:'object',properties:intentProperties,required:Object.keys(intentProperties).filter(key=>key!=='spin'&&key!=='technique'),additionalProperties:false};
...
export function parseShotIntent(value:unknown):ShotIntent{
 const keys=Object.keys(SHOT_INTENT_SCHEMA.properties),required=keys.filter(key=>key!=='spin'&&key!=='technique');
