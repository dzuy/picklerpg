...
export function executeShot(value:unknown,context:ShotContext,players:PlayerState[],conditions:ExecutionConditions):ShotExecution{
 ...
 const quality=clamp(.2+.8*skill/100-.2*difficulty-.2*lowContact-.3*(1-conditions.balance)-contact.penalty-handsPenalty-.1*lobRisk-.25*atpDifficulty-(techniqueIssue?.65:0));
 ...
 const dispersion=benchmark.spread+pressure*benchmark.pressure+.35*atpDifficulty+controlCost*.9;
 ...
 const mishit=techniqueFailed||atpFailed||(random()+1)/2<benchmark.mishit+pressure*benchmark.mishit*2+timing*(1-hitter.skills.hands/100)*.3;
 ...
 return {intended,leg:hitsNet?interceptFlight(leg,t):leg,seed:conditions.seed,quality,skill,difficulty:contact.labels,mishit,dispersion,endpointError:Math.hypot(leg.to.x-intended.aimPoint.x,leg.to.z-intended.aimPoint.z),outcome:techniqueFailed||hitsNet?'net':leg.bounceAtEnd?(out?'out':'in'):'intercept',actualEndpoint};
}
