...
export const APPEARANCE_OPTIONS={presentation:['girl','boy'],face:['oval','round','angular'],hairStyle:['ponytail','short','bun','side-part','bob','curls','spiky','long','none'],hat:['none','cap','backwards','visor','headband','beanie','bucket'],glasses:['none','square','round','sunglasses','sport'],top:['tank','jersey','polo','hoodie'],bottom:['shorts','skirt','skort','pleated-skirt','long-shorts'],accessory:['none','wristband','watch']} as const;
...
export const DEFAULT_APPEARANCE:Appearance={presentation:'boy',face:'oval',hairStyle:'short',hat:'cap',glasses:'none',skin:'#dba67f',hair:'#493629',jersey:'#f3dc86',accent:'#214d43',hatColor:'#214d43',bottomColor:'#214d43',top:'jersey',bottom:'shorts',accessory:'wristband',shoes:'#214d43',paddle:'#214d43'};
...
export function validatePlayer(value:unknown):DesignedPlayer{
 ...
 const appearance={...p.appearance,presentation:p.appearance.presentation??'boy',bottomColor:p.appearance.bottomColor??p.appearance.accent,hatColor:p.appearance.hatColor??(p.appearance.hat==='visor'?p.appearance.jersey:p.appearance.accent),top:p.appearance.top??'jersey',bottom:p.appearance.bottom==='skort'?'skirt':p.appearance.bottom??'shorts',accessory:p.appearance.accessory??'wristband',shoes:p.appearance.shoes??p.appearance.accent,paddle:p.appearance.paddle??p.appearance.accent};
 ...
 for(const key of ['skin','hair','jersey','bottomColor','hatColor','accent','shoes','paddle'] as const)if(!/^#[0-9a-f]{6}$/i.test(appearance[key]))throw new Error('Choose a valid color.');
