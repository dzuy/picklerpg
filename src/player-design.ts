...
export const APPEARANCE_OPTIONS={presentation:['girl','boy'],expression:['happy','determined','angry','crying','serious','confident'],paddleShape:['rectangular','circular','squarish','squarish-stripes','squarish-circles','squarish-lines','rounded','rounded-stripes','rounded-circles','rounded-lines'],face:['oval','round','angular'],hairStyle:['ponytail','short','bun','side-part','bob','curls','spiky','mohawk','long','pigtails','twin-buns','side-braid','long-waves','none'],facialHair:['none','mustache','goatee','short-beard','long-beard','chops'],hat:['none','cap','backwards','visor','headband','beanie','bucket','crown'],glasses:['none','square','round','sunglasses','sport','wraparound','oval','cat-eye','hexagon'],shoeStyle:['court','runner','high-top','slip-on'],top:['tank','jersey','polo','hoodie','long-sleeve'],bottom:['shorts','skirt','skort','pleated-skirt','long-shorts','pants'],accessory:['none','wristband','watch']} as const;
...
function validatePlayerRecord(value:unknown,catchphraseLimit:number):DesignedPlayer{
 ...
 if(p.appearance.lensTranslucency!==undefined&&(!Number.isFinite(p.appearance.lensTranslucency)||p.appearance.lensTranslucency<0||p.appearance.lensTranslucency>100))throw new Error('Lens translucency must be between 0 and 100.');
 ...
 for(const [key,options] of Object.entries(APPEARANCE_OPTIONS))if(!(options as readonly string[]).includes(appearance[key as keyof Appearance] as string))throw new Error('Unknown appearance option.');
 for(const key of ['facialHairColor','lensColor','glassesColor','skin','hair','jersey','bottomColor','hatColor','accent','shoes','paddle'] as const)if(!(key==='lensColor'&&appearance[key]==='none')&&!/^#[0-9a-f]{6}$/i.test(appearance[key]))throw new Error('Choose a valid color.');
