...
export class AvatarThumbnails {
 ...
 get(appearance:Appearance,category:string){
  ...
  const avatar=createAthlete('you',appearance.jersey,appearance);poseAthleteForPortrait(avatar);if(category.startsWith('hand-'))setAthleteHandedness(avatar,category==='hand-left'?'left':'right');this.scene.add(avatar);
  let y=1.145,z=-1.25,x=-.23,targetX=0;
  if(category==='roster'){y=.98;z=-2.35;x=-.48}
