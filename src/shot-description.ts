...
export async function interpretShot(text:string,context:unknown,signal?:AbortSignal):Promise<ParsedCommand>{
 ...
 const response=await fetch('/api/command',{method:'POST',headers:{'Content-Type':'application/json'},signal:signal?AbortSignal.any([signal,AbortSignal.timeout(30000)]):AbortSignal.timeout(30000),body:JSON.stringify({version:1,command,context,options:[{}]})});
