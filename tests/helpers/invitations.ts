...
export function pgInvitations(repo:PgRepository):InviteRepository{
 return {
 async rematchFor(source,actor){return (await repo.query('select * from async_invitations where rematch_of=$1 and $2 in (creator_id,recipient_id)',[source,actor])).rows[0]??null},
