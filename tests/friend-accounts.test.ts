...
test('registration upgrades the authenticated identity, preserves metadata and never creates a replacement user',async()=>{
 ...
 const client:any={auth:{admin:{listUsers:async()=>({data:{users:[user]},error:null}),getUserById:async()=>({data:{user}}),updateUserById:async(id:string,update:any)=>{writes.push({id,update});user={...user,...update,is_anonymous:false};return {data:{user}};},createUser:()=>assert.fail('must never replace a guest')}},from:()=>({insert:async()=>({error:null})})};
 ...
 assert.equal(writes.length,1);assert.equal(writes[0].update.user_metadata.username,'ryan');assert.equal(writes[0].id,A);assert.equal(writes[0].update.app_metadata.friend_guest,true);assert.equal(writes[0].update.email_confirm,true);
