...
for(const court of ['venice','arizona'] as const)test(`invitations preserve ${court} through acceptance and turns`,async()=>{
 ...
 const input={requestId:randomUUID(),opponentId:B,team:[roster.you,roster.partner],court,scoring:'rally-doubles'};
 ...
 assert.equal(first.court,court);assert.equal(first.viewerTeam,'home');assert.equal(first.currentTeam,'home');assert.equal(first.server,'you');assert.equal(first.accountIds?.home,B);assert.equal(first.accountIds?.away,A);
 ...
 const moved=await matches.act(first.id,B,action(first));assert.equal(moved.state.court,court);assert.equal((await matches.get(first.id,A)).court,court);
