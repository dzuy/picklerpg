...
test('public catalog is opt-in, owner-editable, live for later selections, and independent of match snapshots',async()=>{
 ...
 const published=(await as(B,'select * from community_player_catalog()')).rows[0];assert.equal(published.player.name,'Thor');assert.equal(published.player.id,`community-${id}`);assert.equal(published.owner_id,undefined);
