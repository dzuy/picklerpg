...
test('prepared situations produce canonical playable opponent responses',()=>{
 for(const [situation,type] of [['pressure','counter'],['low','reset'],['high','overhead'],['return','return'],['deep','drive'],['dink','dink']]){
  const lab=new PreparedShotFixture();lab.opponentSituation=situation;lab.reset();assert.equal(lab.issue,null,situation);assert.equal(lab.opponentDecision!.intent.type,type,situation);assert.equal(lab.shot.actor,'opponent-left');assert.equal(lab.shot.intent.source,'ai');parseShotIntent(lab.shot.intent);
...
test('style and counter skill change response to the same contact',()=>{
 const lab=new PreparedShotFixture();lab.opponentSituation='pressure';lab.reset();assert.equal(lab.shot.intent.type,'counter');
...
test('policy is deterministic, does not mutate and cannot volley through opening restrictions',()=>{
 const lab=new PreparedShotFixture(),players=lab.state.players,c=preparedContact('counter').context;
...
test('opponent execution and return to manual shot work without stale decision',()=>{
 const lab=new PreparedShotFixture();lab.opponentSituation='high';lab.variance=true;lab.reset();assert.ok(lab.execution);assert.equal(lab.execution!.intended.intent.actor,'opponent-left');
