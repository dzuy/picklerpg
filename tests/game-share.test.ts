...
class Element {
 ...
 addEventListener(name:string,callback:()=>void){const previous=this.events.get(name);this.events.set(name,()=>{previous?.();callback()})}showModal(){this.open=true}close(){this.open=false;this.events.get('close')?.()}remove(){this.removed=true}focus(){this.focused=true}select(){this.selected=true}
...
test('share window supports native sharing, copy fallback, cancellation and reopening',async()=>{
 ...
 try{
  const globals={document:{body,querySelectorAll:()=>[],activeElement:focus,createElement:()=>new Element()},location:{origin:'https://picklebash.app'},navigator,HTMLElement:Element};
  ...
  dialog.close();assert.equal(dialog.removed,true);assert.equal(focus.focused,false);assert.equal(body.focused,true);
