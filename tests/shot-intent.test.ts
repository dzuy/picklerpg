...
import {INPUT_SOURCES,TARGET_ZONES,PLAYER_AIMS,type ShotIntent} from '../src/engine/model';
...
test('all input sources produce identical execution and retain their provenance',()=>{
 ...
 for(const source of INPUT_SOURCES){const sim=new RallyEngine(pressureMiddle);sim.submitIntent({...serve(),source});sim.update(.4);assert.equal(sim.state.shotHistory[0].source,source);if(!firstBall)firstBall=sim.state.ball;else assert.deepEqual(sim.state.ball,firstBall)}
...
test('malformed payloads fail without altering game state',()=>{
 const sim=new RallyEngine(pressureMiddle),before=sim.snapshot();
...
test('offered player-target intents run, but own-team targets are rejected before reset',()=>{
 ...
 const sim=new RallyEngine(provider);sim.submitIntent(sim.availableIntents[0]);assert.deepEqual(sim.state.shotHistory[0].target,target);
...
test('provider intents receive the same structural validation as user input',()=>{
 ...
 assert.throws(()=>new RallyEngine({...pressureMiddle,setup:()=>setup}),/aggression/);
