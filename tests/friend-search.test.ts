...
test('keyboard and pointer selection retain account ID; edits and new names clear it',()=>{
 ...
 try{
  ...
  input.fire('keydown',{key:'ArrowDown'});input.fire('keydown',{key:'Enter'});assert.equal((selected as LobbyTeam|null)?.id,'2');assert.equal(input.value,'pete_plays');assert.equal(hint.textContent,'');
  ...
  search.reset(teams[2]);assert.equal((selected as LobbyTeam|null)?.id,'3');assert.equal(hint.textContent,'');search.reset();assert.equal(selected,null);assert.equal(input.value,'');
