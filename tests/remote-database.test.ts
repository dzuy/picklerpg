 ...
 try{
  ...
  for(const role of ['anon','authenticated']){
   const c=await db.pool.connect();try{await c.query(`set role ${role}`);
    for(const table of ['async_matches','async_match_actions','shot_selection_events']){
     ...
     if(table==='shot_selection_events')await assert.rejects(c.query(`update public.${table} set match_id=match_id`),/permission denied/);
    ...
    await assert.rejects(c.query('select public.backfill_shot_selections(1)'),/permission denied/);
  ...
  await assert.rejects(repository.query("update public.shot_selection_events set completeness='selected_only'"),/permission denied/);
  await assert.rejects(service.act(s.id,B,action(s)),/Wait for your turn/);
  const illegal=structuredClone(action(s));illegal.action.intent.actor='opponent-left';
  await assert.rejects(service.act(s.id,A,illegal),/not legal/);
  const stale=action(s);stale.expectedVersion=10;
  await assert.rejects(service.act(s.id,A,stale));
  assert.equal((await db.pool.query('select count(*) from public.shot_selection_events')).rows[0].count,0);
  ...
  assert.equal((await db.pool.query('select count(*) from public.shot_selection_events')).rows[0].count,1);
  const event=(await db.pool.query('select * from public.shot_selection_events')).rows[0];
  assert.equal(event.chooser_id,A);assert.equal(event.opponent_id,B);assert.equal(event.team,'home');
  assert.equal(event.point_index,0);assert.deepEqual(event.pre_score,{home:0,away:0});
  assert.equal(event.completeness,'complete');assert.deepEqual(event.intent,accepted.action.intent);
  assert.deepEqual(event.capture.offered,s.choices.map(c=>({intent:c.intent,timing:c.timing??null})));
  assert.equal(event.capture.execution.contactOrdinal,1);
  ...
  await repository.setArchived(s.id,A,true);
  ...
  const events=(await db.pool.query('select * from public.shot_selection_events order by to_version')).rows;
  assert.equal(events.length,row.version);assert.ok(events.some(e=>e.chooser_id===B&&e.team==='away'));
  assert.ok(events.some(e=>e.point_result!==null));
  for(const e of events){assert.equal(e.point_index,e.capture.pointIndex);assert.deepEqual(e.point_result,e.capture.execution.pointResult);}
  // Simulate one legacy receipt with no analytics snapshot, then exercise bounded replay.
  await db.pool.query('delete from public.shot_selection_events where match_id=$1 and action_id=$2',[s.id,accepted.actionId]);
  await db.pool.query('update public.async_match_actions set selection_capture=null where match_id=$1 and action_id=$2',[s.id,accepted.actionId]);
  assert.equal((await repository.query('select public.backfill_shot_selections(1) as n')).rows[0].n,1);
  assert.equal((await repository.query('select public.backfill_shot_selections(1) as n')).rows[0].n,0);
  const historical=(await db.pool.query('select * from public.shot_selection_events where action_id=$1',[accepted.actionId])).rows[0];
  assert.equal(historical.completeness,'selected_only');assert.equal(historical.capture,null);assert.equal(historical.point_index,null);
  assert.equal(historical.provenance,'backfill');assert.deepEqual(historical.intent,event.intent);
  // A rolling deployment's old server can still commit; its missing snapshot is explicit.
  const legacyRepository=new PgRepository(db.pool),commit=legacyRepository.commit.bind(legacyRepository);
  legacyRepository.commit=input=>commit({...input,selection:undefined});
  const legacyService=new MatchService(legacyRepository,testers),legacy=await legacyService.create(A,creation());
  const legacyAction=action(legacy);await legacyService.act(legacy.id,A,legacyAction);
  const legacyEvent=(await db.pool.query('select * from public.shot_selection_events where match_id=$1',[legacy.id])).rows[0];
  assert.equal(legacyEvent.completeness,'selected_only');assert.equal(legacyEvent.capture,null);assert.equal(legacyEvent.provenance,'live');
