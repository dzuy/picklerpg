...
test('a receipt newer than the scheduled frame starts playback at zero and skipping cannot change authority',async()=>{
 ...
 const dink={...receipt.state.animation[0],duration:5,pathTimes:undefined,intent:{...receipt.state.animation[0].intent,type:'dink' as const}};assert.equal(samplePlayback(dink,850).progress,1);assert.equal(samplePlayback(dink,425).progress,.5);
