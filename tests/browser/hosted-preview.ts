...
for(const [name,port] of [['a',5178],['b',5179]] as const){
 ...
 const server=await createServer({configFile:false,server:{host:'127.0.0.1',port,strictPort:true,proxy:{'/api/matches':{target:`http://127.0.0.1:${process.env.MULTIPLAYER_PORT||5175}`,changeOrigin:false},'/api/multiplayer':{target:`http://127.0.0.1:${process.env.MULTIPLAYER_PORT||5175}`,changeOrigin:false}}},plugins:[{name:'localhost-hosted-test-signin',configureServer(server){server.middlewares.use((req,res,next)=>{
  ...
  void(async()=>{const client=createClient(process.env.SUPABASE_URL!,process.env.VITE_SUPABASE_PUBLISHABLE_KEY!,{auth:{persistSession:false,autoRefreshToken:false}});const {data,error}=await client.auth.signInWithPassword({email:accounts[name].email,password:accounts[name].password});if(error||!data.session){res.writeHead(503).end('Test sign-in failed.');return;}
   ...
   res.writeHead(200,{'Content-Type':'text/html','Cache-Control':'no-store','Referrer-Policy':'no-referrer'}).end(`<h1>Signing in to hosted test account ${name.toUpperCase()}</h1><script type="module">import {authClient} from '/src/auth-session.ts';const {error}=await authClient().auth.setSession(${session});if(error)document.body.textContent='Sign-in failed';else location.replace('/?multiplayer=1');</script>`);
