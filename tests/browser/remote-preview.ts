...
const handler=createMatchHandler(service,async token=>{if([A,B,C].includes(token))return token;throw new ApiError(401,'authentication','Invalid fixture identity.');});
