...
import {A,B,C,testers} from '../helpers/remote';
...
const handler=createMatchHandler(new MatchService(new PgRepository(db.pool),testers),async token=>{if([A,B,C].includes(token))return token;throw new ApiError(401,'authentication','Invalid fixture identity.');});
const server=await createServer({configFile:false,server:{host:'127.0.0.1',port:5177,strictPort:true},plugins:[{name:'isolated-remote-browser-fixture',enforce:'pre',resolveId(id){if(id.endsWith('/auth-session'))return '\0fixture-auth';},load(id){if(id==='\0fixture-auth')return `const owner=new URLSearchParams(location.search).get('viewer')==='b'?'${B}':'${A}';export const matchCredentials=async()=>({owner,token:owner});export const authClient=()=>({auth:{onAuthStateChange:()=>({data:{subscription:{unsubscribe(){}}}})}});`;},configureServer(server){server.middlewares.use((req,res,next)=>{if(req.url?.startsWith('/api/matches')||req.url?.startsWith('/api/multiplayer'))void handler(req,res);else next();});}}]});
await server.listen();console.log('Disposable remote UI fixture: http://127.0.0.1:5177/?multiplayer=1 (A); add &viewer=b for B.');
