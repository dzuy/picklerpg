...
// Routine soft shots use the menu's clearance, not the lab's tighter default.
function sample(type:ShotType,skill:number){const lab=new PreparedShotFixture();lab.select(type,'typical');if(['drop','dink'].includes(type))lab.shot.intent.intendedNetClearance=.25;for(const k of SKILLS)lab.state.players[0].skills[k]=skill;const c=preparedContact(type).context;c.incomingSpeed=type==='serve'?0:5;let net=0,deep=0,error=0;for(let seed=0;seed<2000;seed++){const e=executeShot(lab.shot.intent,c,lab.state.players,{seed,balance:1});if(e.outcome==='net')net++;if(e.outcome==='in'&&Math.abs(e.actualEndpoint.z)>=4.8)deep++;error+=e.endpointError}return {net:net/2000,deep:deep/2000,error:error/2000}}
