...
import {randomBudgetSkills,fitsSkillBudget} from '../../src/skill-budget';
...
if(!process.argv.includes('--apply')){
 ...
 const plan:Change[]=players!.filter(p=>!hiddenIds.has(p.public_id)).map(p=>{
  let after:PlayerSkills;
  do{after=randomBudgetSkills();}while(JSON.stringify(after)===JSON.stringify(p.published_skills));
 ...
 console.log(JSON.stringify({planned:plan.length,ratings:plan.map(p=>({name:p.name,before:summarizeSkills(p.before).estimatedDupr.toFixed(2),after:summarizeSkills(p.after).estimatedDupr.toFixed(2)}))},null,2));
}else{
