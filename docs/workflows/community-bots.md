...
The normal multiplayer server runs the worker automatically while it is running. It accepts pending invitations (including rematches) after 8–25 seconds and submits legal turns after 4–14 seconds, plus up to 2 seconds polling latency. Timing varies per invitation/turn and survives restarts; version checks and idempotent action IDs protect concurrent workers. Bots use the same invitation and match services as humans. They stop at game completion and do not send chat. Surprise invitations are enabled only for human accounts with server-owned `app_metadata.bot_invites_since` set to an ISO timestamp. The first arrives 15–90 minutes after enabling; later invitations wait 24–72 hours after the most recent bot invitation. Pending bot invitations block new ones. Remove the metadata field to disable. `COMMUNITY_BOTS_ENABLED=false` disables automation without deleting accounts. A continuously running server is required; these are not external scheduled jobs.
