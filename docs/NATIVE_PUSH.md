...
The migration was applied to the production PickleBash Supabase project on September 25, 2026. Both new tables have RLS enabled and deny direct anon/authenticated access. Apple Push Notifications is enabled and the PickleBash Push key exists; Railway configuration and live-device verification are in progress.
...
- [ ] Allow permission. Verify one enabled `push_devices` row for the authenticated account with the expected environment and updated last-seen timestamp. Do not copy tokens into screenshots or logs.
