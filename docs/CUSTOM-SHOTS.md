...
## ATP opportunities in the shot picker

A bounced rally contact outside the net post can offer an ATP when strict trajectory validation confirms a same-side path around the post. The option appears first with a gold “ATP Alert!” card and a short risk/reward description. The player still taps the court to aim. ATP assessment and execution use that exact selected target, including bad angles through the net or post. The alert indicates that a valid route exists from this contact, not that every target is viable. The practice feed also waits for the player to tap; it never opens the selector automatically. Risk depends on existing ATP execution (drive/hands skills and seeded failure); the alert does not guarantee a winner. Serve/return contacts and paths through the post do not show the alert.
