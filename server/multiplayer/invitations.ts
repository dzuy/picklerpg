...
export class InvitationService {
 ...
 async rematch(source:string,actor:string){
  ...
  const row=await this.repo.rematch(source,actor,{id:randomUUID(),creator_id:actor,recipient_id:opponent,team,court:game.court??'forest',scoring:game.rules.scoring as InviteRequest['scoring'],status:'pending',created_at:new Date().toISOString(),match_id:null,request_id:randomUUID(),request_hash:requestHash({source})});
