 ...
 async record(actor:string,target:string,remote:()=>Promise<PublicMatch[]>){
  ...
  const history=async()=>{
   ...
   for(let offset=0;;offset+=500){
    const {data,error}=await this.client.from('match_history').select('id,home_score,away_score,ended_early').eq('owner_id',target).order('id').range(offset,offset+499);
  ...
  return profileRecord(saved,[],games);
 }
