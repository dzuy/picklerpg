-- Stable participant identities plus historical names; old name-only results stay unassigned.
alter table public.match_history add column if not exists participants jsonb;
...
begin
 ...
 ) or (select count(*) from jsonb_array_elements(p_participants)) <> (select count(distinct p->>'player_id') from jsonb_array_elements(p_participants) p)
