-- One rematch request per completed game, shared by both participants.
alter table public.async_invitations add column rematch_of uuid unique references public.async_matches(id);
create function public.create_async_rematch(p_source uuid,p_actor uuid,p_invite jsonb)
...
begin
 ...
 insert into public.async_invitations(id,creator_id,recipient_id,team,court,scoring,request_id,request_hash,rematch_of)
 values((p_invite->>'id')::uuid,p_actor,opponent,p_invite->'team',p_invite->>'court',p_invite->>'scoring',(p_invite->>'request_id')::uuid,p_invite->>'request_hash',p_source) returning * into invitation;
